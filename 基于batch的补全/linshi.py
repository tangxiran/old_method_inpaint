...
if __name__ == '__main__':
    file_dir   = './thin_withmask//'
    file_list  = getFileName(file_dir)
    file_list = file_list[0]
    mask = '../origin_mask.png'

    for pic_one in file_list:
        pic_one_place  = 'linshi.png'
        for importace_number in range(1000,1000+1,1):

            pic_after_inpainted_save_dir = './thin_mask_batchInpainted//importanceX'+str(importace_number)+'_'+ pic_one.replace('.png','')+'//'
            mask = '../origin_mask.png'
            ...
            while( (np.sum(flag == 1)) >0 ):
                ...
                # 保存修复后的照片的位置
                pic_after_inpainted_save_place =   '1000im_inpainted'+str(inpainted_iter_number)+'.png'
                #test
