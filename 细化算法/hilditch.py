...
def hilditch(img_saveplace):
    import cv2
    img = cv2.imread(img_saveplace).astype(np.float32)
...
# hilditch thining
def hilditch_not_toomuch(img_saveplace):
    import cv2
    img = cv2.imread(img_saveplace).astype(np.float32)
...
if __name__ == '__main__':
    ...
    # Read image
    pic = '01.png'
    # img = cv2.imread(pic).astype(np.float32)

    # hilditch thining
    out = hilditch_not_toomuch(img_saveplace=pic)
    print(out)
    # Save result
